...
from yolov3.modeling import build_backbone, build_model
...
def yolov3_darknet53():
    ...
    cfg.MODEL.DARKNETS.NUM_CLASSES = None
    input_shape = ShapeSpec(channels=3, height=256, width=256, stride=32)
    net = build_model(cfg, input_shape)
    #torch.save(net.state_dict(),"./weights/yolov3_darknet53.pth" )
    #net.load_state_dict(torch.load("./weights/yolov3_darknet53.pth"))
    net.eval()
    tensor_out = net(x)

    torch.onnx.export(net,  # model being run
                      x,  # model input (or a tuple for multiple inputs)
                      "./weights/yolov3_darknet53.onnx",  # where to save the model (can be a file or file-like object)
                      operator_export_type=OperatorExportTypes.ONNX_ATEN_FALLBACK,
                      # verbose=True,  # NOTE: uncomment this for debugging
                      # export_params=True,
                      )
    ...
    import onnx
    import onnxruntime
    onnx_model = onnx.load("./weights/yolov3_darknet53.onnx")
    onnx.checker.check_model(onnx_model)

    ort_session = onnxruntime.InferenceSession("./weights/yolov3_darknet53.onnx")
    ...
    # compare ONNX Runtime and PyTorch results
    np.testing.assert_allclose(to_numpy(tensor_out[0]), ort_outs[0], rtol=0.2, atol=1e-05)
